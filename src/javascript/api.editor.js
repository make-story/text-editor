	...
	var bundle = (function() {
		...
		EditBundle.prototype = {
			...
			*/
			...
			// 이미지 비율 리사이즈
			getAspectRatioFit: function(originalWidth, originalHeight) {
				/*
				var maxWidth = 0;
				var maxHeight = 0;
				...
				if(scale > 1) scale = 1;
				var resizeWidth = scale * originalWidth;
				var resizeHeight = scale * originalHeight;
				*/
				...
				*/
				...
				// 가로, 세로 최대 사이즈 설정
				var maxWidth = originalWidth * 0.5;   // 원하는대로 설정. 픽셀로 하려면 maxWidth = 100  이런 식으로 입력
				var maxHeight = originalHeight * 0.5;   // 원래 사이즈 * 0.5 = 50%
				var resizeWidth;
				var resizeHeight;
				if(maxWidth < originalWidth || maxHeight < originalHeight) {
					...
					if(originalHeight < originalWidth) {
						...
						resizeHeight = Math.Round((originalHeight * resizeWidth) / originalWidth);
					}else {
						// 세로가 가로보다 크면 세로는 최대사이즈로, 가로는 비율 맞춰 리사이즈
						resizeHeight = maxHeight;
						resizeWidth = Math.Round((originalWidth * resizeHeight) / originalHeight);
					}
				}else {
					// 최대사이즈보다 작으면 원본 그대로
